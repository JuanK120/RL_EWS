...
if __name__ == '__main__':
    ...
    for repeat in range(REPEATS):
        ...
        knowledge = [[config, 0, 1, 0] for config in configurations] 

        # Get environment information (EWS)
        state = []
        count = 0
        ...
        input_size = state.shape  # state shape
        output_size = len(knowledge) #action space
        reward_size = 2 # number of objectives

        print("Information about environment, state shape:", input_size, " action space:", output_size,
            "number of objectives:", reward_size)
        ...
        for _ in range(EPISODES):
                ...
                for parent_conn, action in zip(parent_conns, actions):
                    ...
                    new_configuration_i = knowledge[action] 
                    eRI.change_configuration(configurations[action])
                    chosen_confs_indexes.append(action)
                    chosen_confs.append(configurations[action])
                    cost_of_episode = cost_dict[configurations[action]] 
                    ...
                    while (time.time() - start_time) < COLLECTION_WINDOW:
                        ...
                        if(reading):
                            ...
                            reqCount+=1 
                    knowledge[action][REWARD] += sum(sample_list) 
                    knowledge[action][N_K] += reqCount
                    knowledge[action][COST] = cost_of_episode
                    ...
                    rds.append(False)
                    mrws.append(reward)
                    scrs.append(np.sum(reward))

                next_states, rewards, dones, real_dones, morewards, scores = [], [], [], [], [], []
                cnt = 0
                for parent_conn in parent_conns:
                    s, r, d, rd, mor, sc = ns,rws,ds,rds,mrws,scrs
